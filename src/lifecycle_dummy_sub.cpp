...
{
public:
  explicit LifecycleListener(const std::string & node_name,std::string pub_name)
  : Node(node_name)
  {
    lifecycle_node_name = node_name;
    // Data topic from the lc_talker node
    sub_data_ = this->create_subscription<std_msgs::msg::String>(pub_name+"_topic", 10,std::bind(&LifecycleListener::data_callback, this, std::placeholders::_1));
  ...
  {
    RCLCPP_INFO(get_logger(), "%s data_callback: %s",lifecycle_node_name, msg->data.c_str());
  }
...
int main(int argc, char ** argv)
{
  ...
  auto lc_listener = std::make_shared<LifecycleListener>(name,pub_name);
