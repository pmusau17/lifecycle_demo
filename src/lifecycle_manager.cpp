...
{
public:
  explicit LifecycleServiceClient(const std::string & node_name, const std::string & managed_name)
  : Node(node_name)
  {
      managed_lifecycle_node = managed_name;
  }
  ...
  init()
  {
    ...
    client_get_state_ = this->create_client<lifecycle_msgs::srv::GetState>(managed_lifecycle_node+"/get_state");
    client_change_state_ = this->create_client<lifecycle_msgs::srv::ChangeState>(managed_lifecycle_node+"/change_state");
  }
  ...
  get_state(std::chrono::seconds time_out = 3s)
  {
    ...
    if (future_status != std::future_status::ready) {
      RCLCPP_ERROR(
        get_logger(), "Server time out while getting current state for node %s", managed_lifecycle_node);
      return lifecycle_msgs::msg::State::PRIMARY_STATE_UNKNOWN;
    }
    ...
    if (future_result.get()) {
      RCLCPP_INFO(
        ...
        managed_lifecycle_node, future_result.get()->current_state.label.c_str());
      return future_result.get()->current_state.id;
    } else {
      RCLCPP_ERROR(
        get_logger(), "Failed to get current state for node %s", managed_lifecycle_node);
      return lifecycle_msgs::msg::State::PRIMARY_STATE_UNKNOWN;
    }
  }
...
private:
  ...
  std::string managed_lifecycle_node;
};
...
int main(int argc, char ** argv)
{
  ...
  auto lc_client = std::make_shared<LifecycleServiceClient>(name,managed_name);
  lc_client->init();
